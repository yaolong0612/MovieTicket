...
public class TicketDiscountCalculatorFactoryTest extends BaseTest {
    ...
    private static Stream<Arguments> parameters() {
        return Stream.of(
            Arguments.of(
                Ticket.builder().ticketType(TicketType.ADULT).quantity(generateRandomInt(1, Integer.MAX_VALUE)).build(),
                NoDiscountCalculator.class),
            Arguments.of(
                Ticket.builder().ticketType(TicketType.TEEN).quantity(generateRandomInt(1, Integer.MAX_VALUE)).build(),
                NoDiscountCalculator.class),
            Arguments.of(Ticket.builder().ticketType(TicketType.CHILDREN).quantity(generateRandomInt(1, 3)).build(),
                NoDiscountCalculator.class),
            Arguments.of(
                Ticket.builder().ticketType(TicketType.SENIOR).quantity(generateRandomInt(1, Integer.MAX_VALUE))
                    .build(), SeniorDiscountCalculator.class),
            Arguments.of(
                Ticket.builder().ticketType(TicketType.CHILDREN).quantity(generateRandomInt(3, Integer.MAX_VALUE))
                    .build(), ChildrenDiscountCalculator.class)
        );
    }
